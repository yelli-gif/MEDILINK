...
export default function StaffDirectory() {
  ...
  useEffect(() => {
    ...
    let personnel = [...DEFAULT_STAFF];
    if (savedStaff) {
      try {
        const parsed = JSON.parse(savedStaff) as StaffMember[];
        const combined = [...parsed, ...DEFAULT_STAFF];
        personnel = combined.filter((s, index, self) => 
    ...
    // 2. Load Services for the filter
    ...
    let allDepts = [...defaultServiceNames];
    
    if (savedServices) {
      try {
        const parsed = JSON.parse(savedServices);
        const customDepts = parsed.map((s: any) => s.name);
        allDepts = [...new Set([...allDepts, ...customDepts])];
      } catch (e) {
