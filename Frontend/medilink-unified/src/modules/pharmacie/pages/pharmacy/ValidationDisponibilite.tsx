...
import { pharmacieAPI, consultationAPI } from '../../../../services/api';
...
export default function ValidationDisponibilite(): React.JSX.Element {
  ...
  const handleSend = async () => {
    ...
    try {
      // Lot 5 - Pharmacie API (port 8085)
      await pharmacieAPI.validerDemande(parseInt(id));
      ...
      console.error("Erreur validation pharma:", err);
      // Fallback for demo
      updatePrescriptionStatus(id, 'Validé');
      setIsSending(false);
      setIsSuccess(true);
      setTimeout(() => navigate('/pharmacie/dashboard'), 2000);
    }
  };
